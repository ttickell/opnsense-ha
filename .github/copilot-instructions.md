...
- **Check the cable first.** Before debugging any code or configuration, verify that the surrounding infrastructure (simulators, upstreams, dependencies, network services) is actually doing what it is supposed to do. A symptom in your code is often caused by something outside your code. Validate the environment end-to-end before touching the codebase.
...
- **Confirm root cause before editing.** Before modifying any project file to fix a symptom, identify and run the specific diagnostic command that directly confirms the hypothesized root cause. If running that command does not reproduce or confirm the failure, the hypothesis is wrong — stop and look upstream. Do not edit code based on a plausible theory alone.
